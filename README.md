...
Tablite 是一个面向 Chromium 浏览器的 Manifest V3 新标签页扩展。它把新标签页变成一个干净的搜索入口：中间只有搜索框，背景使用必应每日一图，搜索引擎可以在 Google、Baidu 和自定义 URL 模板之间切换。它不接入分析、广告或遥测服务，设置保存在浏览器本地，默认配置开箱即用。
